...
export function DashboardTab() {
  ...
  const METRIC_INFO = {
    avgLeads: {
      description: 'Total leads created divided by the number of active days.',
      advice: '💡 Increase campaign volume or improve targeting to hit your daily goal.',
    },
    avgInvitations: {
      description: 'Total LinkedIn invitations sent per active day.',
      advice: '💡 Ensure you are sending the daily limit (approx. 15) to maintain a healthy pipeline.',
    },
    avgConnections: {
    ...
    invToConvRate: {
      ...
      advice: '💡 Aim for 0.75%. Improve your messaging and follow-up sequences to increase conversions.',
    },
    connToConvRate: {
  ...
  const getColorClass = (value, target) => {
    if (typeof value !== 'number' || typeof target !== 'number' || target === 0) return 'text-gray-800';
      ...
      {!loading && selectedAgentId && activeChannel === 'linkedin' && (
        <div className="space-y-4">
          {(() => {
            ...
            const inboundKeywords = ['CSR', 'CUSTOMER SUCCESS', 'CUSTOM SERVICE', 'INBOUND'];
            ...
            const avgLeads = linkedInMetrics.leads / activeDays;
            const avgInvitations = linkedInMetrics.invitations / activeDays;
            const avgConnections = linkedInMetrics.connections / activeDays;
            ...
            return (
              <>
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4" id="dailyConnectionsContainer">
                  ...
                  <div className="w-full h-64"><canvas id="dailyConnectionsChart"></canvas></div>
                </div>
                ...
                {leadsTimeseries && leadsTimeseries.length > 0 && (
                  <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
                    ...
                    <div className="w-full h-64"><canvas id="leadsChart"></canvas></div>
                  </div>
                )}
                ...
                {!isCrs && (
                  <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
                    ...
                    <div className="w-full h-64"><canvas id="funnelChart"></canvas></div>
                  </div>
                )}
        ...
        <div className="space-y-4">
          ...
          {emailsSentTimeseries && emailsSentTimeseries.length > 0 && (
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
              ...
              <div className="w-full h-64"><canvas id="emailsSentChart"></canvas></div>
            </div>
          )}
          {replyRateTimeseries && replyRateTimeseries.length > 0 && (
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
              ...
              <div className="w-full h-64"><canvas id="replyRateChart"></canvas></div>
            </div>
          )}
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
            <h3 className="text-sm font-semibold text-gray-700 mb-3">Email Funnel</h3>
            <div className="w-full h-64"><canvas id="emailFunnelChart"></canvas></div>
          </div>
